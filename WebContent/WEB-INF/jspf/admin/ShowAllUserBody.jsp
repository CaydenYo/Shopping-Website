...
<table border="1" cellspacing="0"
		...
		<tr>
			<td colspan="12"
				...
				<input type="button" value="查询" onclick="GetUserBySday_Eday()"/>
			</td>
		</tr>
		<tr>
		...
		<c:forEach items="${sessionScope.listallusers}" var="list">
		<tr align="center">
		<td>${list.uid }</td>
		<td>${list.name }</td>
		<td>${list.username}</td>
		<td>${list.gender eq "1"?"男":"女" }</td>
		<td>${list.idCard }</td>
		<td>${list.age }</td>
		<td>${list.address }</td>
		<td>${list.telno }</td>
...
<script type="text/javascript">
	...
	//	$("#endtime").datepicker();
	//});
	function GetUserBySday_Eday(){
		//data="currentPage="+currentPage;
		//$.ajax({
		//	type : "post",
		//	async : true,
		//	url:"admin/queryUsersByDate",
		//	dataType : "text",
		//	cache : false,
		//	data : {
		...
		window.location="admin/queryUsersByDate?starttime="+starttime+"&endtime="+endtime;
	}
